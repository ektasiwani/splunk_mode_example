...
if(isset($_GET['search_text'])){
 $name1= $_GET['search_text'];
 $name= mysqli_real_escape_string($con,trim($name1));
$query = "SELECT * FROM quiz_takers WHERE username='".$name."'  ";
...
while ($rows = mysqli_fetch_array($result))
{
 ...
 $col++;
  ...
  echo '</tr>';
  echo '<tr>';
  echo '<td>', '<center>','<br>', mysqli_real_escape_string($con,$rows[1]) ,'<br>','<br>','<br>','<br>', '</center>', '</td>';
